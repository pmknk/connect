...
export class Env {
    ...
    number(key: string, defaultValue?: number): number {
        return this.env[key] ? parseInt(this.env[key]!) : defaultValue || NaN;
    }
    ...
    boolean(key: string, defaultValue?: boolean): boolean {
        return this.env[key] ? this.env[key] === 'true' : defaultValue || false;
    }
    ...
    array(key: string, defaultValue?: any[]): any[] {
        return this.env[key] ? this.env[key]!.split(',') : defaultValue || [];
    }
    ...
    object(key: string, defaultValue?: object): object {
        return this.env[key] ? JSON.parse(this.env[key]!) : defaultValue || {};
    }
