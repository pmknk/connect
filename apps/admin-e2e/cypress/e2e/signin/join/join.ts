...
Given(
    'the join request is mocked as success with alias "joinRequest"',
    () => {
        cy.interceptOk('POST', JOIN_URL,  {
            data: {
                success: true
            }
        }, 'joinRequest');
...
When('I fill the confirm password form with Confirm Password {string}', (confirmPassword: string) => {
