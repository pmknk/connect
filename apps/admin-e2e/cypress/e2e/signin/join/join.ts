...
Given('the invite request is mocked as not found with alias {string}', (alias: string) => {
    cy.interceptNotFound('GET', INVITE_URL, { message: 'Not Found' }, alias);
});
...
Given('the invite request is mocked as ok with code {string} and alias {string}', (code: string, alias: string) => {
...
When('I type password {string} and confirm password {string}', (password: string, confirm: string) => {
    typeJoinPassword(password);
    typeJoinConfirmPassword(confirm);
});

Given('the signup request is mocked as created with alias {string}', (alias: string) => {
    cy.interceptCreated('POST', SIGNUP_URL, { data: { success: true } }, alias);
});
